...
int main(int argc, char **argv){
    // Allocate dict to be contiguous in memory
    int num_dict = atoi(argv[1]);
    struct dictionaries* dict = malloc(sizeof(struct dictionaries));
    ...
    for (int blk_idx=0; blk_idx<128; blk_idx++) {
        for (int sec_idx=0; sec_idx<128; sec_idx++) {
            for(int i=0; i <5;i++) {
                dict->dict[i].section[sec_idx].block[blk_idx].data[0] = sec_idx;
            }
        }
    }
    ...
    for (int repeat=0; repeat<10000; repeat++) {
        for (int blk_idx=0; blk_idx<128; blk_idx++) {
            ...
            for (int sec_i=0; sec_i<128; sec_i++) {
                ...
                for(int i=0; i < 5;i++) { 
                    int data = dict->dict[i].section[sec_idx].block[blk_idx].data[d_idx];
